...
from pyspark.sql.functions import col, desc, lag, rank
...
top_10_countries = df_march.groupBy("date","location").sum("new_cases")
top_10_countries = top_10_countries.orderBy(desc("sum(new_cases)")).limit(10)

windowSpec = Window.partitionBy("date").orderBy(desc("sum(new_cases)"))
top_10_countries = top_10_countries.withColumn("rank", rank().over(windowSpec))
top_10_countries = top_10_countries.filter(col("rank") <= 10).drop("rank")
...
top_10_countries = top_10_countries.withColumnRenamed("sum(new_cases)", "количество новых случаев")
...
df_russia = df.filter((df.location == "Russia") & (df.date >= "2021-03-25") & (df.date <= "2021-03-31"))
